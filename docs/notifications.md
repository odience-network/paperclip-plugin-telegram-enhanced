...
| **Issue blocked** | Sent when an issue becomes blocked **and** is owned by a human/board user (`assigneeUserId` set). Agent-only blocks are suppressed. Off by default (`notifyOnIssueBlocked`). |
| **Board mention** | Sent when an issue comment `@`-mentions a configured board username — case-insensitive and word-boundary aware. Off by default; requires `notifyOnBoardMention` + `boardUsernames`. |
...
| **Agent run started / finished** | Per-run lifecycle notifications. Off by default (high-frequency on busy instances). |
