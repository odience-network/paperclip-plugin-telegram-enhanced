 ...
 *
 * The parked state is marked resolved before the remaining steps run. That
 * ordering is the point: it makes a second press of the same button a no-op
 * instead of running the tail of the workflow twice. Telegram leaves the
 * buttons on screen and re-delivers callbacks it considers unacknowledged, so a
 * double press is the expected case, not the unlucky one.
 */
export async function resolveWorkflowApprovalCallback(
  ...
  // Mark resolved rather than deleting: writing a JS `null` value hits a NOT
  // NULL constraint on plugin_state.value_json at the platform layer (BLA-606)
  // and throws before either Telegram call below runs, so the button silently
  // does nothing. A sentinel object keeps the write valid.
